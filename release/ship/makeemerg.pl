...
# TopDir is where this script is being run from
$TopDir = `pwd`; 
chomp($TopDir);
...
if ( $FIRSTARG = "emergency" ) {
  print "Auto generating emergency key\n";
}   
...
#  $prompt =~ s/"/\\"/g;
#  $custName = $prompt;
  $custName = "GemFire Emergency License";
...
sub make_emerg_license {
  local($luout, $keepgoing );
  local( @alltypes ) = ("evaluation", "development", "production"); 
...
#    $licType = "Evaluation";
    $licType = "$type";
    $PropFileName = "emergency.${licType}.license";
    
    unlink("$TempDir/$PropFileName");
    $luout=`$GEMFIRE/../hidden/bin/lu daysToDate 35`;
    ( $first, $second ) = split ('\n', $luout );
    $sunsetDate = $second;
