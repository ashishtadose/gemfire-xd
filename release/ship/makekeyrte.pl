...
#     to a single file named "gfRTELicense.zip" that it creates in
#     the current directory. If a "gfRTELicense.zip" already exists
#     it will be deleted.
# 
...
#   date, node, native-node, cpus, purchased-cpus, and hyperthreaded
# 
...
#   for. Legal values are: "Solaris", "Linux", and "Windows".
# 
...
# 
# hyperthreaded: If specified indicates if the customer claimed they
#   had hyperthreaded cpus on a cpu limited node locked license.
#   If not specified then cpus are not hyperthreaded. If such a
#   claim was made then cpus should not equal purchased-cpus.
#   Currently if this property is true cpus should be twice the
#   number of purchased-cpus.
# 
#
#-------------------------------------------------------------
#
...
# todo: bullet proof the hyperthreading question
...
local ($sunsetDate, $hypertheaded, $nNode, $licType);
...
if ($status == 1) {
  ...
  while ( $another == "0") { 
    ...
    if ("$action" eq "q") {
      ...
      &dump_license("$CustDir/gfRTELicense.zip");
...
if ($rdytosign == 1) {
  ...
  if ($rollgid == 1) {
    $hid++;
    print "Rolling group-id to $hid which is now the highest group-id.\n";
  }
  ...
  # At this point we have a gemfireLicense.zip that needs to be 
  # renamed to gfRTELicense.zip for the RTE product. This is done 
  # in save_licenses subroutine, I hope
  ...
  if ( $sstatus != 0) {
    ...
    # save the original gfRTELicense.zip, if exists, to new name appending
    ...
    print "\nNew gfRTELicense.zip file created at:\n";
    print "   $TopDir/$CustDir/gfRTELicense.zip\n\n\n";
...
sub save_licenses {
  ...
  $status=0;
  if (-f "$CustDir/gfRTELicense.zip") {

    # Name to archive old zip
    $OldCustZip = &dateAsExtension(time) . ".zip";
  
    &message( "sign_license", "Found existing gfRTELicense.zip. Renaming it to $OldCustZip" );

    $status=&copyfile( "$CustDir/gfRTELicense.zip", "$CustDir/$OldCustZip" );
    if ($status != 0) {

    &message( "sign_license", "Problem copying $CustDir/gfRTELicense.zip to $CustDir/$OldCustZip");

    die "\nFailure: copying $CustDir/gfRTELicense.zip to $CustDir/$OldCustZip\n";
    }
  }
  
  # Save new gfRTELicense.zip from tmp to archive
  $status=&copyfile( "$TempDir/gemfireLicense.zip", "$CustDir/gfRTELicense.zip" );
  if ($status != 0) {
    &message( "sign_license", "Problem copying $TempDir/gfRTELicense.zip to $CustDir/gfRTELicense.zip." );
    print "\nFailure: copying $TempDir/gfRTELicense.zip to $CustDir/gfRTELicense.zip\n";
    $cleanup="no";
  }
...
sub unzip_license {
  ...
  $zipout=`unzip $CustDir/gfRTELicense.zip -d $TempDir`;
...
sub dump_license {
  ...
  $savetag = 0;
  @licout=`/ship5/users/ship/shipgfv10/test/product/bin/gemfire license -file=$filename`;
...
sub get_platform {
  ...
  if ( "$licType" ne "evaluation" ) {
    while ( $gpflag == "0") {
      ...
      print "Enter either [s]olaris, [l]inux, [w]indows: [s|l|w] ";
      $prompt = <STDIN>;
      chop($prompt);
      if ( ("$prompt" eq "s") || ("$prompt" eq "l") || ("$prompt" eq "w")) {
...
sub calc_eval_days {
  ...
  while ( $calcflag == "0") {
    ...
    print "\nThat sets the sunset date to $second ";
...
sub make_prop_file { 
  ...
  print KEY "hyperthreaded=$hypertheaded\n";
...
sub get_cpu_info { 
  ...
  print "\nDoes this machine use hypertheaded CPUs? [y/n] ";
  $prompt = <STDIN>;
  chop($prompt);
  if (($prompt eq "y") || ($prompt eq "Y")) {
    $hypertheaded = "$prompt";
    &message( "get_cpu_info", "setting hypertheaded to $hypertheaded" );
  }
}
...
sub start_new_license {
  ...
  if (("$prompt" eq "e")) {
    ...
    if (("$prompt" eq "p")) {
      ...
      &message( "start_new_license", "unlmited development license choice" );
      $licType="development";
