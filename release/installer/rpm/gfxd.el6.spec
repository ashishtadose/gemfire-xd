...
%define GPHDHome    /usr/lib/gphd
%define InstallDir  %{GPHDHome}/%{InstallDirName}
%define DataDir     /var/lib/gphd/%{name}
...
%define GFXD_LOG_DIR    /var/log/gphd/%{name}
%define GFXD_LOG_NAME   gfxdserver.log 
# Init scripts
%define InitBuildScript  gfxd.init
%define InitProdScript   gfxd
# names for the sysconfig file
%define ConfigFileBuildName gfxd.sysconfig
...
# Add symlink to /usr/lib/gphd/gfxd
ln -sf %{InstallDir} %{GPHDHome}/%{ProductName}
...
if [ $1 = 0 ]; then
   ...
   if [ -h %{GPHDHome}/%{ProductName} ]; then
      rm -f %{GPHDHome}/%{ProductName}
   fi
...
%posttrans
 ...
 if [ -d %{GPHDHome}/Pivotal_GemFireXD_05Beta2 ]; then
    rm -rf %{GPHDHome}/Pivotal_GemFireXD_05Beta2
 fi
...
* Fri Jan 31 2014 Pivotal Support %{version}-%{release}
- GemFire XD 1.0
