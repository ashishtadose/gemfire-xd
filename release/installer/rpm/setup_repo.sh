...
#For example, if the file located at /tmp/PRTS-1.4.0, then the created repo file will be /etc/yum.repos.d/PRTS-1.4.0.repo;
...
if [ ${RETVAL} -eq 0 ]; then
  echo "PRTS YUM Repo file successfully created at ${YUM_REPO_DIR}/${REPO_NAME}.repo"
  echo "Use http://${FQDN}/${REPO_NAME} to access the repository."
else
  echo "Failed to created PRTS YUM Repo: ${MESSAGE}"
fi
