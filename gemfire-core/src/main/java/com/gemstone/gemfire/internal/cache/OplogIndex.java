...
public final class OplogIndex {
  ...
  public static final byte[] INDEX_END_OF_FILE_MAGIC  = new byte[] { INDEX_END_OF_FILE, -0x37, -0x11, -0x26, -0x46, 0x25, 0x71, 0x3b, 0x1f, 0x4b, -0x77, 0x2b, -0x6f, -0x1f, 0x6b, -0x02};
  ...
  private long getIndexFileVersion(String indxFile) {
    ...
    if(!matcher.matches()) {
      throw new InternalGemFireError("Could not match index file pattern against " + indxFile);
    }
    String versionStr = matcher.group(1);
    long fileVersion = Long.valueOf(versionStr);
    return fileVersion;
  }
  
  private File getFileName(long version) {
    return new File(oplog.getDirectoryHolder().getDir(), oplog.getDiskFileName() + "." + version + IDX_FILE_EXT);
  }
  ...
  synchronized void initializeForWriting(boolean truncate) throws IOException {
      ...
      if (reason != null) {
        ...
        if (logger.infoEnabled()) {
          logger.info(LocalizedStrings.Oplog_DELETE_0_1_2, new Object[] {
              this.oplog.toString(), this.irf.getAbsolutePath() + '(' + reason + ')',
              this.dsi.getName() });
        }
      }
      if (!this.irf.delete()) {
        ...
        logger.warning(LocalizedStrings.Oplog_DELETE_FAIL_0_1_2, new Object[] {
            this.oplog.toString(), this.irf.getAbsolutePath(), this.dsi.getName() });
      }
    }
  }
  ...
  public static final class IndexData {
    ...
    IndexData(final SortedIndexContainer index,
      ...
      this.indexEntryMap = (action != ONLY_LOAD ? new THashMapWithCreate(
          entryCacheSize) : null);
      this.action = action;
    }
    ...
    public boolean equals(Object other) {
      if (other instanceof IndexData) {
        return this.index.equals(((IndexData)other).index);
      }
      else {
        return false;
      }
  ...
  public void writeIndexRecords(List<KRFEntry> entries,
    ...
    final int entryCacheSize = (int)Math.min(BATCH_FLUSH_SIZE_AT_ROLLOVER,
        entries.size());
    final HashMap<Long, IndexData[]> drvIdToIndexes =
        new HashMap<Long, IndexData[]>();
    ...
    final boolean hasOffHeap = getDiskIdToIndexDataMap(dumpIndexes, loadIndexes,
        entryCacheSize, drvIdToIndexes, null);
    ...
    final THashMapWithCreate.ValueCreator entryListCreator =
      ...
      public Object create(Object key, Object params) {
        if(hasOffHeap) {
          //Snapshot the key bytes, as the offheap value bytes used as index key would be
    ...
    if (dumpIndexes != null) {
      for (SortedIndexContainer index : dumpIndexes) {
        ...
        if (indexJob != null) {
          ...
          data = new IndexData(index, null, IndexData.ONLY_DUMP, entryCacheSize);
        }
        indexData.add(data);
      }
    }
  ...
  public boolean getDiskIdToIndexDataMap(Set<SortedIndexContainer> dumpIndexes,
    ...
    for (DiskRegionInfo regionInfo : recoveredRegions) {
      ...
      if (!hasOffHeap && ((IndexData)indexSet.firstKey()).index.getBaseRegion()
  ...
  public void recoverIndexes(
    ...
    try {
      ...
      final HashMap<String, IndexData> indexMap =
          new HashMap<String, IndexData>();

      boolean endOfFile = false;

      String currentIndexID = null;
      ...
      while (!endOfFile) {
        ...
        switch (opCode) {
          case INDEX_END_OF_FILE:
            ...
            if(!Arrays.equals(data, INDEX_END_OF_FILE_MAGIC)) {
              throw new DiskAccessException("Did not find end of file magic at the end of index " + this.irf, oplog.getParent());
            }
            break;
          ...
          case INDEX_RECORD:
            ...
            int numRegionKeys = (int)InternalDataSerializer.readUnsignedVL(in);
            ...
            for (int i = 0; i < numRegionKeys; i++) {
              ...
              if (diskId != null && diskId.getOplogId() == this.oplog.oplogId) {
                if (logEnabled) {
                  logger.info(LocalizedStrings.DEBUG, String.format(
                      "OplogIndex#recoverIndexes: adding index diskEntry=%s "
                          + "indexContainer=%s indexKeyBytes=%s", diskEntry,
                      currentIndex, Arrays.toString(indexKeyBytes)));
                }
                ...
                if (logEnabled) {
                  logger.info(LocalizedStrings.DEBUG, String.format(
                      "OplogIndex#recoverIndexes: skipping adding index for diskEntry=%s "
                          + " in indexContainer=%s as oplogid=%s doesn't match entry's oplogid=%s",
