...
testcase=com.gemstone.sqlfabric.jdbc.SimpleAppTest.txt

if [ "$testcase" != "" ]; then
  testcase=`echo $testcase | sed -e 's/\.txt//' | sed -e 's#\.#/#g'`
  testcase=-Dsqlf.junit.testcase=${testcase}.class
fi

echo "testcase is $testcase"

bash build.sh sqlf-run-junit-tests $testcase -Dlocal.conf=$SRC/dunit.conf
