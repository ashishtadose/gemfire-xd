...
testcase=com.pivotal.gemfirexd.internal.engine.distributed.PreparedStatementDUnit.txt
...
if [ "$testcase" != "" ]; then
  ...
  testcase=-Dgfxd.dunit.testcase=${testcase}.class
fi
echo "testcase is \"$testcase\""

bash build.sh gfxd-run-dunit-tests $testcase -Dlocal.conf=$SRC/dunit.conf
#bash build.sh gfxd-run-dunit-tests $testcase -Dlocal.conf=$SRC/local.conf
