...
// GemStone changes BEGIN
	  try {
	    return PropertyUtil.getServiceProperty(
	        Misc.getMemStoreBootingNoThrow(), key, null);
	  } catch (StandardException se) {
            if (Monitor.reportOn || GemFireXDUtils.TraceAuthentication) {
              SanityManager.DEBUG_PRINT(AuthenticationTrace, "getProperty(" + key
                  + ") received standard exception ", se);
            }
            else {
              SanityManager.DEBUG_PRINT(AuthenticationTrace, "AuthenticationServiceBase:getProperty(" + key
