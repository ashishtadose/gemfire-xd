...
/**
 * Created by vbhaskar on 30/11/15.
 */
public class PrepStatementSnappyActivation extends GemFireSelectDistributionActivation {
  ...
  public PrepStatementSnappyActivation(ExecPreparedStatement eps,
    ...
    //System.out.println("PrepStatementSnappyActivation created for sql=" + sql + " ,stack=" + ExceptionUtils.getFullStackTrace(new Throwable()));
  }
      ...
      if (this.pvs.getParameterCount() > 0) {
        ...
        if (GemFireXDUtils.TraceQuery) {
          SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_QUERYDISTRIB,
              "PrepStatementSnappyActivation.executeWithResultSet. modified-sql=" + querySql);
        }
      } else {
        if (GemFireXDUtils.TraceQuery) {
          SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_QUERYDISTRIB,
              "PrepStatementSnappyActivation.executeWithResultSet. No dynamic parameters for sql=" + this.sql);
        }
        querySql = this.sql;
      }
      SnappyActivation.executeOnLeadNode((SnappySelectResultSet)rs, rc, querySql, enableStreaming);
    } else {
      throw StandardException.newException(
          SQLState.LANG_UNEXPECTED_USER_EXCEPTION, "Not a prepared statement. Sql=" + this.sql + " ,isPrepStmt=" + this.getIsPrepStmntQuery() + " ,pvs=" + this.pvs);
    }
  }
        ...
        if (tokenList.size() != 0) {
          if (tokenList.size() == this.pvs.getParameterCount()) {
            if (GemFireXDUtils.TraceQuery) {
              SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_QUERYDISTRIB,
                  "PrepStatementSnappyActivation.getModifiedSql. Got sql=" + sql
                      + " ,#tokens=" + tokenList.size() + " ,#params" + this.pvs.getParameterCount()
                      + " ,pvs=" + this.pvs
                      + " ,tokens=" + tokenList);
            }
            int startPos = 0;
            ...
            for (final Token v : tokenList) {
              ...
              modifiedSqlStr.append(str).append(" ").append(pvs.getParameter(i++).toString()).append(" ");
            }
            ...
            errorMsg = "For SQL: " + sql + " ,Unequal: #tokens=" + tokenList.size() + " ,#params=" + this.pvs.getParameterCount() + " ,tokens=" + tokenList + " ,params=" + this.pvs;
          }
        } else {
          errorMsg = "For SQL: " + sql + " , 0 tokens." + " #params" + this.pvs.getParameterCount() + " ,params=" + this.pvs;
        }
      } else {
        errorMsg = "For SQL: " + sql + " , Null token list." + " #params" + this.pvs.getParameterCount() + " ,params=" + this.pvs;
      }
    } else {
      errorMsg = "Not GenericPreparedStatement SQL: " + sql + " ,but " + this.preStmt.getClass().getSimpleName();
    }
    if (GemFireXDUtils.TraceQuery) {
      SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_QUERYDISTRIB,
